...
export class System implements Entity<SystemId> {
  ...
  constructor(
    ...
    this.isBoundary = obj.actorType !== undefined &&
      obj.actorType == "boundary";
...
export class SystemsAndComponents {
  ...
  private constructor(
    ...
    this.noneAggregateComponents = components.filter((v) =>
      !v.isSystemAggregated
    );
  }

  static create(
    systems: Entities<SystemId, System>,
    components: Entities<ComponentId, Component>,
  ) {
    // 整合の確認
  ...
  aggregateSystem(): SystemsAndComponents {
    const aggregatedComponents = new Entities<ComponentId, Component>(
      this.components.map((v) => v.aggregateSystem()),
    );
  ...
  aggregateSystemWithoutBoundary(): SystemsAndComponents {
    const aggregatedComponents = new Entities<ComponentId, Component>(
      this.components.map((v) => v.isBoundary ? v : v.aggregateSystem()),
    );
    return new SystemsAndComponents(this.systems, aggregatedComponents);
  }
  ...
  findSystemAndBoundary(): Entities<SystemIdOrComponentId, SystemOrComponent> {
    ...
    this.systems.map((v) => SystemOrComponent.ofSystem(v)).forEach((v) =>
      list.push(v)
    );
    this.components.filter((v) => v.isBoundary).map((v) =>
      SystemOrComponent.ofComponent(v)
    ).forEach((v) => list.push(v));
    return new Entities<SystemIdOrComponentId, SystemOrComponent>(list);
  }

  findBoundaryComponentIdOrSystemId(
    systemIdOrComponentId: SystemIdOrComponentId,
  ): SystemIdOrComponentId {
    if (this.cache.contains(systemIdOrComponentId.stringValue)) {
      return systemIdOrComponentId;
    }
    const isSystem = this.systems.contains(
      new SystemId(systemIdOrComponentId.stringValue),
    );
    if (isSystem) {
      return this.cache.put(
        systemIdOrComponentId.stringValue,
        systemIdOrComponentId,
      );
    }

    const isComponent = this.components.contains(
      new ComponentId(systemIdOrComponentId.stringValue),
    );
    if (!isComponent) {
      throw new Error("不明なID: " + systemIdOrComponentId.stringValue);
    }

    const component = this.components.findById(
      new ComponentId(systemIdOrComponentId.stringValue),
    );
  ...
  findComponentIdOrSystemId(
    ...
    const c = this.components.findById(
      new ComponentId(systemIdOrComponentId.stringValue),
    );
  ...
  findByComponentIdOrSystemId(
    ...
    if (isSystem) {
      return SystemOrComponent.ofSystem(
        this.systems.findById(new SystemId(systemIdOrComponentId.stringValue)),
      );
    }
    const isComponent = this.components.contains(
      new ComponentId(systemIdOrComponentId.stringValue),
    ...
    return SystemOrComponent.ofComponent(
      this.components.findById(
        new ComponentId(systemIdOrComponentId.stringValue),
      ),
    );
  }
  ...
  findAll(): SystemOrComponent[] {
    return [
      ...
      ...this.noneAggregateComponents.values.map((v) =>
        SystemOrComponent.ofComponent(v)
      ),
    ];
  }
