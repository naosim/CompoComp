...
export class PlanUmlConverter {
  convert(models: Models, options?: {aggregateType?: AggregateType, displayUsecaseName?: boolean, title?: string}): string {
    ...
    if(aggregateType == AggregateType.none) {
      systemsAndComponents.findAll().filter(v => v.isComponent || v.isSingleSystem()).forEach((v) => plantuml.push(toPlantUml(v, models.componentStyles)))
    } else {
...
export function toPlantUml(v: SystemOrComponent, componentStyles: ComponentStyles): string {
